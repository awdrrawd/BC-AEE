...
async function adoptForeignDrawing(slot: Slot, item: Item): Promise<void> {
  ...
  const id = `${owner}:${ref.o}:${ref.s}:${ref.r}`;
  const currentTarget = () => Player === player && Player.MemberNumber === owner && player.Appearance.includes(item)
    && getActiveSession()?.slot !== slot;
...
export async function slotLoad(i: number) {
  ...
  if (!isCurrentSession(session)) { endEditSession(session); return; }
  if (!loaded) {
...
function rollbackSession(session: SlotEditSession) {
  ...
  if (initialState) {
    ...
    C.Appearance = C.Appearance.filter(value => value.Asset.Group.Name !== slot.maskGroup && value.Asset.Group.Name !== slot.visGroup);
    for (const original of session.companions) {
      C.Appearance.splice(Math.min(original.index, C.Appearance.length), 0, original.item);
    }
...
export async function applyToCharacter(): Promise<boolean> {
  ...
  const session = getActiveSession();
  if (!session || session.phase !== 'editing' || !isCurrentSession(session)) return false;
  ...
  try {
    if (!State.useSps) {
      compressed = session.hasDrawing ? canvasEmbeddedData(slot.canvas) : "";
  ...
  if (!useSps && projected >= APPEARANCE_UPLOAD_BYTES) {
    ...
    if (!isCurrentSession(session)) {
      if (getActiveSession() === session) { slotExit(); closeFocusedItem(); }
      return false;
    }
    if (!useSps) {
