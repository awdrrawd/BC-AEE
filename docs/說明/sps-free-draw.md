...
編輯器的 160K 容量條在兩種模式皆顯示整份 AccountUpdate 的預估大小。SPS 模式以實際 v4 短引用結構（網址、擁有者、槽位、64 字元 hash、MIME 與版本）取代目前格的內嵌圖片來估算，不編碼圖片；其他格維持原有儲存格式。確認上傳時仍檢查每張圖片的 3,300,000 bytes 上限。
