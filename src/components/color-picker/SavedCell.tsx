...
export function SavedCell({item, selected, onClick}: { item: SavedColor; selected: boolean; onClick: () => void }) {
  return <button
    className={`relative aspect-square rounded border bg-[repeating-conic-gradient(#333_0%_25%,#222_0%_50%)] bg-[length:8px_8px] ${selected ? 'border-[var(--aee-accent)]' : 'border-zinc-700 hover:border-zinc-500'}`}
    onClick={onClick}>
