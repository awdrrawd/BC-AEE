...
export function ControlButton({active, label, icon, onClick}: {active: boolean; label: string; icon: ReactNode; onClick: () => void}) {
  return <button className={`${ctrlButtonBaseClass} ${active ? '[&_.aee-frame]:brightness-125 [&_.aee-frame]:hue-rotate-160' : ''}`} title={label} onClick={onClick}>
