...
export function beginEditSession(slot: Slot, character: Character, item: Item): SlotEditSession {
  ...
  activeSession = {
    ...
    snapshot: null,
    initialState: null,
    companions: character.Appearance.flatMap((value, index) =>
      value.Asset.Group.Name === slot.maskGroup || value.Asset.Group.Name === slot.visGroup
        ? [{index, item: {...value, Property: value.Property ? CommonCloneDeep(value.Property) : undefined}}] : []),
  };
...
export function isCurrentSession(session: SlotEditSession | null | undefined): session is SlotEditSession {
  return !!session && activeSession === session && Player === session.player
    && Player?.MemberNumber === session.member && session.character.Appearance.includes(session.item);
}
...
export function isEditorInteractive(): boolean {
  return isCurrentSession(activeSession) && activeSession.phase === 'editing';
}

export function isSessionPreviewReady(session: SlotEditSession | null): session is SlotEditSession {
  return isCurrentSession(session) && (session.phase === 'editing' || session.phase === 'saving');
}

export function endEditSession(session: SlotEditSession): boolean {
  if (activeSession !== session) return false;
  session.slot._loadToken = (session.slot._loadToken ?? 0) + 1;
  session.slot._loadedSig = undefined;
  session.slot._loadingSig = undefined;
  session.slot._loadPromise = undefined;
