...
const GRID = 'grid-cols-[34px_76px_minmax(120px,1fr)_92px_92px_64px]';
...
export function WardrobeMigrationDialog({onClose}: { onClose: () => void }) {
  ...
  const slots = useMemo(() => {
    void dataVersion;
    return scanWardrobeMigration(source, character);
  }, [source, character, dataVersion]);
  ...
  const focusedAfter = focused
    ? buildWardrobeMigrationOutfit(focused, character, part => selected(focused, part))
    : null;
  ...
  const setPart = (slot: WardrobeMigrationSlot, part: WardrobeMigrationPart, value: boolean) =>
    setChoices(previous => ({...previous, [partKey(slot.index, part)]: value}));
  const setSlot = (slot: WardrobeMigrationSlot, value: boolean) => setChoices(previous => {
    const next = {...previous};
    for (const part of slot.parts) next[partKey(slot.index, part)] = value;
    return next;
  });
  ...
  return <Dialog onDismiss={onClose} className="h-240 w-490 p-6">
    ...
    <div className="flex min-h-0 flex-1 gap-5">
      ...
      <section className="flex min-h-0 flex-1 flex-col gap-2">
        ...
        <div className="aee-scroll flex min-h-0 flex-1 flex-col overflow-y-auto rounded-xl border border-white/20">
          <div className={cn('sticky top-0 z-10 grid shrink-0 items-center gap-2 border-b border-white/20 bg-zinc-950 px-4 py-2 text-center text-[25px] leading-none text-white', GRID)}>
            <span/><span>{t('wardrobe-migration-column-label')}</span>
            <span className="text-left">{t('wardrobe-migration-column-name')}</span>
            <span>{t('wardrobe-migration-mode-none')}</span>
            <span>{t('wardrobe-migration-after')}</span>
            <span>{t('wardrobe-migration-column-total')}</span>
          </div>
          {slots.length ? <div className="flex shrink-0 flex-col gap-1 px-2 pb-2 pt-1">
            {slots.map(slot => <MigrationSlotRow key={slot.index} slot={slot}
              focused={focused?.index === slot.index} open={expanded.has(slot.index)}
              selected={part => selected(slot, part)} onFocus={() => setFocusSlot(slot.index)}
              onToggle={() => toggleExpanded(slot.index)} onPart={(part, value) => setPart(slot, part, value)}
              onSlot={value => setSlot(slot, value)}/>)}</div>
...
function MigrationSlotRow({slot, focused, open, selected, onFocus, onToggle, onPart, onSlot}: {
  slot: WardrobeMigrationSlot; focused: boolean; open: boolean;
  selected: (part: WardrobeMigrationPart) => boolean;
  onFocus: () => void; onToggle: () => void;
  onPart: (part: WardrobeMigrationPart, value: boolean) => void;
  onSlot: (value: boolean) => void;
}) {
  const corrected = slot.parts.filter(selected).length;
  const none = slot.parts.length - corrected;
  return <div className={cn('shrink-0 overflow-hidden rounded-lg border', focused ? 'border-(--aee-accent)' : 'border-white/8')}>
    <div className={cn('grid cursor-pointer items-center gap-2 bg-white/3 px-2 py-2 text-center', GRID)}
      ...
      <button className={cn('text-[25px] leading-none', none ? 'text-white' : 'text-zinc-600')}
              onClick={event => {event.stopPropagation(); onSlot(false); onFocus();}}>{none}</button>
      <button className={cn('text-[25px] leading-none', corrected ? 'text-white' : 'text-zinc-600')}
              onClick={event => {event.stopPropagation(); onSlot(true); onFocus();}}>{corrected}</button>
      <span className="text-[26px] leading-none text-white">{slot.parts.length}</span>
    </div>
    {open ? <div className="border-t border-white/10">{slot.parts.map(part => {
