...
const firstPage=Array.from({length:1000},(_,i)=>`key${i}`);
let page=0;
handler=async()=>Response.json({ok:true,keys:page++===0?firstPage:['key999','last'],cursor:page===1?'a+/=':null});
assert.equal((await exports.listSpsKeys()).length,1001);
assert.ok(calls.at(-1).url.endsWith('?cursor=a%2B%2F%3D'));
handler=async()=>Response.json({ok:true,keys:[],cursor:'same'});
await assert.rejects(exports.listSpsKeys(),/invalid_cursor/);
handler=async()=>Response.json({ok:true,keys:[1],cursor:null});
await assert.rejects(exports.listSpsKeys(),/invalid_key_page/);
console.log('SPS multi-page listing, cursor encoding and malformed response checks passed');
