...
仍需完成遊戲內驗收、伺服器條件提交或等效的跨裝置衝突機制、舊記錄的安全清理。SPS cursor 分頁契約已核對並補上測試。請測試不同 AEE 版本共存、網路中斷及其他模組在繪圖預覽期間保存角色的情境。需求來源見 [dev 功能訴求與移植計畫](./dev-review.md)。
...
- 已提供目前選取衣櫃的手動批次遷移：先下載備份，所有圖片上傳成功後才替換引用。其他衣櫃與目前穿戴人物不自動掃描；仍待真實遊戲驗收。
- 來源：[SPS 自由繪圖](../說明/sps-free-draw.md)。

## 3. 圖片與雲端記錄資源管理

- SPS 繪圖來源改用最多 96 筆的 Data URL LRU，避免撤銷仍被 BC 引用的 Blob URL；本地匯入仍正常撤銷短期 URL。
- 尚需實機量測 BC／遮罩等其他快取的總記憶體。雲端舊圖片與衣櫃記錄保留；已加入唯讀衣櫃封存、未引用紀錄報告，以及透過既有匯入預覽還原的路徑，尚不執行刪除。跨裝置競爭與服務端一致性限制仍待解決。
