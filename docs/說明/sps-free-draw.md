...
在衣櫃設定的儲存頁選「備份並遷移舊繪圖」。確認後會先觸發標準衣櫃 JSON 備份下載，再將目前來源中三個 AEE DrawingBoard 的 CustomDraw 圖片上傳至 SPS 公開圖片空間。請確認瀏覽器確實保留下載檔。只接受 PNG／WebP data URL 或原有 LZString 壓縮格式，不改動第三方道具的同名屬性。

所有圖片上傳完成後，才將衣櫃資料改為 v4 圖片引用並保存。上傳失敗不替換原衣櫃內容；帳號切換或來源內容改變會停止。已上傳的圖片不因失敗而自動刪除。保存結果不確定時請保留備份並重新載入衣櫃確認。BC 線上來源仍只能確認更新已排入遊戲佇列。

此操作只處理目前選取的衣櫃，不會掃描其他衣櫃、目前穿戴人物或其他玩家。舊版 AEE 可能無法讀取 v4 引用。新增提示提供繁中、簡中與英文，其他語言暫沿用英文 fallback。
