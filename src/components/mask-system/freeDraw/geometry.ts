...
// Swatch colour for the toolbar. Carries State.alpha so the colour button and
// the stroke-size dot actually SHOW the current opacity — otherwise it's an
// invisible setting living only inside the picker panel.
export function colorForFill(hex: string, alpha = 1): string {
  if (alpha < 1) {
